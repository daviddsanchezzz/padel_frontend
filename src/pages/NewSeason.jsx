...
const NewSeason = () => {
  ...
  useEffect(() => {
    getNewSeasonPreview(id)
      .then((res) => { setPreview(res.data); setSeason(res.data.nextSeason); })
      .catch(() => setError('No se puede generar la vista previa. ¿Hay clasificaciones disponibles?'))
      .finally(() => setLoading(false));
  }, [id]);

  const handleCreate = async () => {
    if (!confirm(`¿Crear la temporada "${season}"? Se generará una nueva competición con los equipos redistribuidos.`)) return;
    setCreating(true); setError('');
    try {
      const res = await createNewSeason(id, { season });
                  ...
                  {allTeams.length === 0 ? (
                    ...
                    <div className="divide-y divide-gray-50">
                      {allTeams.map((team) => {
                        ...
                        return (
                          <div key={team._id} className={`px-5 py-2.5 flex items-center justify-between ${
                            isPromoted  ? 'bg-brand-50' :
                            isRelegated ? 'bg-red-50'   : ''
                          }`}>
                            <div className="flex items-center gap-2.5">
                              <span className="text-xs text-gray-400 w-4 text-right">{team.position}</span>
                              <span className={`text-sm font-medium ${
                                isPromoted  ? 'text-brand-700' :
                                isRelegated ? 'text-red-600'   : 'text-gray-700'
                              }`}>
                                {team.name}
                              </span>
                            </div>
                            <span className={`text-[11px] font-bold ${
                              isPromoted  ? 'text-brand-600' :
                              isRelegated ? 'text-red-500'   : 'text-gray-400'
                            }`}>
                              {isPromoted  ? '↑ Sube' :
                               isRelegated ? '↓ Baja' : '—'}
                            </span>
