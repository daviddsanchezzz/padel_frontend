...
import { Building2, CreditCard, Search, Trophy, Users } from 'lucide-react';
    ...
    <AppLayout title="Organizaciones">
      ...
      <div className="mb-4">
        <div className="relative w-full md:max-w-lg">
          ...
          <input
            ...
            placeholder="Buscar organización o propietario..."
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="hidden md:grid grid-cols-[minmax(0,1.2fr)_110px_minmax(0,0.8fr)_90px_minmax(0,1fr)] px-4 py-2 bg-gray-50 border-b border-gray-200">
            {['Organización', 'Tipo', 'Propietario', 'Info', 'Deportes'].map((h) => (
          ...
          {filtered.map((org) => {
            ...
            return (
              <div
                key={org.id}
                className="grid grid-cols-1 md:grid-cols-[minmax(0,1.2fr)_110px_minmax(0,0.8fr)_90px_minmax(0,1fr)] gap-2 md:gap-3 items-start px-4 py-3 border-b border-gray-100 last:border-b-0"
              >
                {/* Nombre + slug */}
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5 truncate ml-6">/{org.slug}</p>
                </div>
                ...
                {/* Propietario */}
                <div className="min-w-0">
                  <p className="text-sm text-gray-800 font-medium truncate">{org.ownerName || '—'}</p>
                  {org.ownerEmail && (
                    <p className="text-xs text-gray-400 truncate">{org.ownerEmail}</p>
                  )}
                </div>

                {/* Competiciones + miembros */}
                <div className="flex flex-col gap-1">
                  <span className="inline-flex items-center gap-1 text-xs text-gray-600">
                    <Trophy size={11} className="text-gray-400" />
                    {org.activeCompetitions} activas
                  </span>
                  <span className="inline-flex items-center gap-1 text-xs text-gray-600">
